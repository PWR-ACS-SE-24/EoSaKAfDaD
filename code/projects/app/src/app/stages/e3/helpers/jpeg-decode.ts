...
type DecodeFn = (component: ComponentT, zz: Int32Array, spectralStart: number, spectralEnd: number, successive: number) => void;
type LeafOrTree = number | LeafOrTree[];
type HuffmanTableT = {children: LeafOrTree, index: number};


interface ComponentT  { blocksPerLine: number; blocksPerColumn: number; quantizationTable: any; blocks: Int32Array[][]; huffmanTableDC: LeafOrTree; huffmanTableAC: LeafOrTree; pred: number; v: number, h: number, quantizationIdx: number; lines: Uint8Array[], scaleX: number, scaleY: number};
interface FrameT { components: ComponentT[]; samplesPerLine: number; scanLines: number; maxH: number; maxV: number; mcusPerLine: number; mcusPerColumn: number; extended: boolean, progressive: boolean; precision: number; componentsOrder: number[]};
interface Opts { tolerantDecoding: boolean }
...
export class JPEGDecoder {
  ...
  public getDCTEmbeddedData(embedLength: number): Uint8ClampedArray {
    return Uint8ClampedArray.from(this.secretData.flatMap((block) => block.slice(0, embedLength)).flatMap(pixel => (pixel & 0b10) >> 1))
  }

  constructor(protected data: Uint8ClampedArray, userOpts: Partial<Opts> = {}) {
    let defaultOpts: Opts = { tolerantDecoding: true };
    this.opts = {...defaultOpts, ...userOpts};
  }

  private buildHuffmanTable(codeLengths: Uint8Array, values: Uint8Array): LeafOrTree {
    let k = 0, length = 16;
  ...
  private decodeBaseline(component: ComponentT, zz: Int32Array, spectralStart: number, spectralEnd: number, successive: number) {
    let block = []
  ...
  private decodeDCFirst(component: ComponentT, zz: Int32Array, spectralStart: number, spectralEnd: number, successive: number) {
    var t = this.decodeHuffman(component.huffmanTableDC);
    var diff = t === 0 ? 0 : (this.receiveAndExtend(t) << successive);
    zz[0] = (component.pred += diff);
  }

  private decodeDCSuccessive(component: ComponentT, zz: Int32Array, spectralStart: number, spectralEnd: number, successive: number) {
    zz[0] |= this.readBit() << successive;
  }

  private decodeACFirst(component: ComponentT, zz: Int32Array, spectralStart: number, spectralEnd: number, successive: number) {
    if (this.eobrun > 0) {
      this.eobrun--;
    }
  ...
  private decodeACSuccessive(component: ComponentT, zz: Int32Array, spectralStart: number, spectralEnd: number, successive: number) {
    ...
    while (k <= e) {
      ...
      case 2:
        if (zz[z])
          zz[z] += (this.readBit() << successive) * direction;
        else {
          r--;
          if (r === 0)
  ...
  private decodeMcu(mcusPerLine: number, component: ComponentT, decode: DecodeFn, mcu: number, row: number, col: number, spectralStart: number, spectralEnd: number, successive: number) {
    ...
    if (component.blocks[blockRow] === undefined && this.opts.tolerantDecoding)
      return;
    decode(component, component.blocks[blockRow][blockCol], spectralStart, spectralEnd, successive);
  }

  private decodeBlock(component: ComponentT, decode: DecodeFn, mcu: number, spectralStart: number, spectralEnd: number, successive: number) {
  ...
  private decodeScan(frame: {mcusPerLine: number; mcusPerColumn: number; progressive: boolean},
                     components: ComponentT[], resetInterval: number,
    ...
    if (progressive) {
      if (spectralStart === 0)
        decodeFn = successivePrev === 0 ? this.decodeDCFirst : this.decodeDCSuccessive;
      else
        decodeFn = successivePrev === 0 ? this.decodeACFirst : this.decodeACSuccessive;
    } else {
    ...
    while (mcu < mcuExpected) {
      ...
      if (componentsLength == 1) {
        ...
        for (n = 0; n < resetInterval; n++) {
          this.decodeBlock(component, decodeFn, mcu, spectralStart, spectralEnd, successive);
          mcu++;
        }
      } else {
        for (n = 0; n < resetInterval; n++) {
          for (i = 0; i < componentsLength; i++) {
            ...
            for (j = 0; j < v; j++) {
              for (k = 0; k < h; k++) {
                this.decodeMcu(mcusPerLine, component, decodeFn, mcu, j, k, spectralStart, spectralEnd, successive);
              }
            }
          }
          mcu++;
  ...
  private quantizeAndInverse(zz: Int32Array, dataOut: Uint8Array, dataIn: Int32Array, component: ComponentT) {
  ...
  private buildComponentData(component: ComponentT) {
    ...
    for (var blockRow = 0; blockRow < blocksPerColumn; blockRow++) {
      ...
      for (var blockCol = 0; blockCol < blocksPerLine; blockCol++) {
        this.quantizeAndInverse(component.blocks[blockRow][blockCol], r, R, component);
    ...
    while (fileMarker != 0xFFD9) { // EOI (End of image)
      ...
      switch(fileMarker) {
        ...
        case 0xFFDB: // DQT (Define Quantization Tables)
          ...
          let quantizationTablesEnd = quantizationTablesLength + this.offset - 2;
        ...
        case 0xFFC2: // SOF2 (Start of Frame, Progressive DCT)
          ...
          let componentsCount = this.data[this.offset++], componentId;
        ...
        case 0xFFFF: // Fill bytes
          if (this.data[this.offset] !== 0xFF) { // Avoid skipping a valid marker.
            this.offset--;
          }
          break;
        default:
          if (this.data[this.offset - 3] == 0xFF &&
              this.data[this.offset - 2] >= 0xC0 && this.data[this.offset - 2] <= 0xFE) {
            ...
            if (malformedDataOffset !== -1) {
              throw new Error(`first unknown JPEG marker at offset ${malformedDataOffset.toString(16)}, second unknown JPEG marker ${fileMarker.toString(16)} at offset ${(this.offset - 1).toString(16)}`);
            }
