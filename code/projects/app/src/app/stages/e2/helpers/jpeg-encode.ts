...
export class JPEGEncoder {
  ...
  constructor(quality: number) {
    // init huffman tables
    this.YDC_huffman_table = this.computeHuffmanTbl(
      STD_DC_LUMINANCE_NRCODE,
      STD_DC_LUMINANCE_VALUES,
    );
    this.UVDC_huffman_table = this.computeHuffmanTbl(
      STD_DC_CHROMINANCE_NRCODES,
      STD_DC_CHROMINANCE_VALUES,
    );
    this.YAC_huffman_table = this.computeHuffmanTbl(
      STD_AC_LUMINANCE_NRCODES,
      STD_AC_LUMINANCE_VALUES,
    );
    this.UVAC_huffman_table = this.computeHuffmanTbl(
  ...
  private initQuantTables(sf: number) {
    ...
    for (let row = 0; row < 8; row++) {
      for (let col = 0; col < 8; col++) {
        this.Y_forward_table[k] =
          1.0 / (this.YTable[ZIGZAG_PATTERN[k]] * AASF[row] * AASF[col] * 8.0);
        this.UV_forward_table[k] =
          1.0 / (this.UVTable[ZIGZAG_PATTERN[k]] * AASF[row] * AASF[col] * 8.0);
        k++;
      }
    }
  }
  ...
  private processDU(
    current_block_values: Array<number>,
    forward_table: Array<number>,
    DC: number,
    huffman_table_DC: Array<[number, number]>,
    huffman_table_AC: Array<[number, number]>,
    secret_message: Array<number>,
    embedSize: number,
  ) {
    ...
    for (
      let j = 0;
      j < end0pos && j < embedSize && secret_message.length;
      j++
    ) {
      this.DU[j + 1] =
        (this.DU[j + 1] ^
          ((this.DU[j + 1] & 0b11) + (secret_message.shift()! << 1))) |
        1;
  ...
  public encode(
    ...
    data: { secret: Array<number>; embedSize: number },
  ) {
    ...
    while (y < height) {
      ...
      while (x < quadWidth) {
        ...
        for (pos = 0; pos < 64; pos++) {
          ...
          this.Y_current_block_values[pos] =
            0.299 * r + 0.587 * g + 0.114 * b - 128; //-0x80
          this.U_current_block_values[pos] =
            -0.16874 * r + -0.33126 * g + 0.5 * b;
          this.V_current_block_values[pos] =
