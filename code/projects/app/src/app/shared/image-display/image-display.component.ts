...
import { ImageLocalStorageService } from "../image-local-storage/image-local-storage.service";
...
export class ImageDisplayComponent implements OnInit {
  ...
  public addImage(which: "left" | "right") {
    ...
    this.imageLocalStorage.addImage(which, this.currentImage).then(
      (result) => this.flash(which, result ? "green" : "red")
    );
  }
}
